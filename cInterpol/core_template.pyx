...
cdef extern void ${token}_coeff${i}(const double t[], const double y[], double c[], const size_t nt)
%endfor
%endfor
...
%for token in tokens
cpdef Piecewise_${token} Piecewise_${token}_from_coefficients(t, c, allow_extrapol=True):
    """ Classmethods not supported by cdef extension types, this is a proxy function """
...
cdef class Piecewise_${token}:
    """
    for `poly`: mimics PiecewisePolynomial class (mimics scipy.interpolate.PiecewisePolynomial)
    """
    cdef public double [:] t
    cdef public double [:,:] c
    cdef public int wy # width of y matrix (number of derivatives per y point incl 0th)
    ...
    def __cinit__(self, double [:] t, double [:,:] y, _c = None,
        ...
        if _c != None:
            ...
            self.wy = y.shape[1]
            if self.wy == 1:
                ${token}_coeff1(&t[0], &y[0,0], &c[0,0], len(t))
